...
sub np {
    ...
    if ($MPD{'artist'} eq "" and $MPD{'title'} eq "" and $MPD{'album'} eq "") {
        $str = $MPD{'alt_text'};
    } else {
        $str = $MPD{'format'};
    }
...
Irssi::settings_add_str('mpd', 'mpd_format', '♫ %ARTIST - %TITLE [%ALBUM] ♫');
Irssi::settings_add_str('mpd', 'mpd_alt_text', '♫ %FILENAME ♫');

Irssi::command_bind mnp        => \&np;
